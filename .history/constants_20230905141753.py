...
ADDITIONAL_DETAILS = "1 mutation, FPB scaled exp function, symmetry: if metInfo >= 0 and stepValue > 0: self.fitness += (((2-metInfo)**2)+0.5); else: self.fitness -= stepValue * 0.5"
# Can be COS or EXP
FIT_FUNCTION = "COS"
# General COS:
    # "if stepValue > 0: self.fitness += (metInfo * np.cos(((2*np.pi)/metInfo)*timestep))"
# General EXP:
    # "if metInfo >= 0 and stepValue > 0: self.fitness += (((2-metInfo)**2)+0.5); else: self.fitness -= stepValue * 0.5"
