...
class ROBOT:
        ...
        if c.OPTIMIZE_AGE == False and c.SECOND_OBJ == "SYMMETRY":
            ...
            for sensor in self.sensors:
                for sensorB in self.sensors:
                    if sensor.name < sensorB.name:
                        summedDiff += -1 * abs(sensor.numSteps - sensorB.numSteps)
    ...
    def Sense(self, timestep, framesPerBeat):
        stepValue = 0
        for sensor in self.sensors:
        ...
        """if stepsToClick >= 0 and stepValue > 0:    
            self.fitness += (((2-stepsToClick)**2)+0.5)
