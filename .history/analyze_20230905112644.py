...
class ANALYZE:
    ...
    def Run_Analysis(path, fitness=True, steps=True, bar=True, rand=True):
        ...
        if rand == True:
            ...
            fig.hist(bestFitness, label="Best Solution", orientation="horizontal", color = "red", height=0.5)
            randomPopulation = ageFitnessArray[0, :, :]
            randSolutions = (randomPopulation[:, 0])
            fig.hist(randSolutions, label="Random Solutions", bins=30, orientation="horizontal", alpha=0.5)
            fig.legend()
            plt.gca().set(title='Frequency Histogram', xlabel='Fitnesses')
            fig.set_title("Distribution of random solution fitnesses")

            plt.savefig(path + "plots/Rand.png")


ANALYZE.Run_Analysis("bestRuns/50sols_50gens/run64/", bar=False)